...
{
    public class IncreaseResourceSupplyCommandHandler : BaseSectorHandler,INotificationHandler<IncreaseResourceSupplyCommand>
    {
        public IncreaseResourceSupplyCommandHandler(IMongoRepository<SectorDocument, Guid> sectorDocuments, IMediator mediator, ISectorService sectorService, IMongoRepository<SectorResourcesDocument, Guid> sectorResourcesDocuments) : base(sectorDocuments, mediator, sectorService, sectorResourcesDocuments)
        {
        }

        public async Task Handle(IncreaseResourceSupplyCommand notification, CancellationToken cancellationToken)
        {
            lock(ResourceLocker.Lock)
            {
                ...
                foreach (var resource in notification.Resources)
                {
                    ...
                    sectorResource.Amount += resource.Amount;
                }
